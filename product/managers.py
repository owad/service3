...
class OutdatedManager(models.Manager):
    def get_out_of_date(self, status, user):
        ...
        if status == 'moje':
            product_ids = Comment.objects.filter(user=user, status=Product.PROCESSING,
                                                 type=Comment.STATUS_CHANGE).values_list('product_id', flat=True)
            return Product.objects.filter(id__in=product_ids)
        elif status == 'przeterminowane':
            return Product.objects.filter(Q(updated__lte=datetime.now() - timedelta(days=3),
                                            status__in=(Product.NEW, Product.PROCESSING, Product.COURIER)) |
            ...
            return products.filter(Q(updated__lte=datetime.now() - timedelta(days=3),
                                     status__in=(Product.NEW, Product.PROCESSING, Product.COURIER)) |
                                   Q(updated__lte=datetime.now() - timedelta(days=7), status=Product.READY) |
                                   Q(updated__lte=datetime.now() - timedelta(days=10), status=Product.EXTERNAL))
    ...
    def get_for_report(self, get_data):
        ...
        if get_data:
            ...
            closed_products = Comment.objects.filter(status=Product.CLOSED,
                                                     type=Comment.STATUS_CHANGE,
                                                     created__range=(startdate, enddate)).values_list('product_id',
                                                                                                      flat=True)
